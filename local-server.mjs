...
app.get('/api/local/audio', (req, res) => {
  try {
    ...
    if (!fsSyc.existsSync(filePath)) {
      res.status(404).json({ error: 'File not found' });
      return;
    }

    const stat = fsSyc.statSync(filePath);
    ...
    if (range) {
      ...
      fsSyc.createReadStream(filePath, { start, end }).pipe(res);
    } else {
      res.writeHead(200, {
        'Content-Length': fileSize,
        'Content-Type': 'audio/mpeg',
      });
      fsSyc.createReadStream(filePath).pipe(res);
    }
  } catch (error) {
